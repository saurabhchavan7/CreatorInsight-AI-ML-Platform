...
# logging configuration (keep yours as-is)
...
def build_pipeline(max_features: int, ngram_range: tuple, learning_rate: float, max_depth: int, n_estimators: int):
    pipeline = Pipeline([
...
def main():
    try:
        ...
        pipeline = build_pipeline(max_features, ngram_range, learning_rate, max_depth, n_estimators)

        logger.debug("Fitting pipeline (TF-IDF + LightGBM)...")
        pipeline.fit(X_train, y_train)
        logger.debug("Pipeline training complete.")
        ...
        # Save ONE artifact
        pipeline_path = os.path.join(models_dir, "sentiment_pipeline.pkl")
        with open(pipeline_path, "wb") as f:
            pickle.dump(pipeline, f)

        logger.debug("Pipeline saved to %s", pipeline_path)

    except Exception as e:
        logger.error('Failed to complete pipeline training: %s', e)
        print(f"Error: {e}")
